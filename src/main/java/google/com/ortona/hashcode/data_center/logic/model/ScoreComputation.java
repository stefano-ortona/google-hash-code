...
public class ScoreComputation {
	...
	public int computeTotalScore(Datacenter center, int poolNum) {
		List<Integer> poolScores = new ArrayList<>(poolNum);
		for (int poolId = 0; poolId < poolNum; poolId++) {
			poolScores.add(computeScore(poolId, center));
		}
		return Collections.min(poolScores);
	}
